...
| `packaging/` | `aur/hypr-swap/PKGBUILD` and `aur/hypr-swap-bin/PKGBUILD`, the two Arch recipes — one compiles the release, one installs its prebuilt binary. The `.deb` and `.rpm` recipes are not here — they are `[package.metadata.deb]` and `[package.metadata.generate-rpm]` in `Cargo.toml`, so that both read the one `version` |
