...
| FR-088 E2E runs against a supplied compositor | **Developer machine — requirement unmet** | there is no `e2e` job: both routes for supplying automation a compositor were built and measured and neither hosts a nested one (R29, **failed**). Recorded as a deviation at FR-088; the tier runs on a contributor's own session, [quickstart.md](./quickstart.md) scenario 5, optionally through the pinned image of R30 |
