...
| FR-088 E2E runs against a supplied compositor | **Developer machine, pending route 2** | the `e2e` job runs the image of R29/R30 on every change but does **not** gate: route 1 gives automation a session and no GPU, so the nested compositor never gets a monitor (R29, measured). Recorded as a deviation at FR-088; the gating tier is a contributor's own session, [quickstart.md](./quickstart.md) scenario 5 |
